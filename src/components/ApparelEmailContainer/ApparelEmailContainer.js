    ...
    return (
        <Fragment>
            <div className={styles.EmailContainer}>
                <input type="mail" className={`${styles.Input} ${visibilityStatus ? styles.Danger : null}`} ref={inputRef} onClick={() => { updatedMsg(false); }} onChange={visibilityHandler} placeholder="Email Address" required />
                <button type="submit" onClick={visibilityHandler}>
                    <figure className={styles.EmailButtonFigure}>
                        <img src={IconArrow} alt="submit button" className={styles.EmailButtonSVG} />
                    </figure>
                </button>
                {visibilityStatus ?
                ...
                {visibilityStatus ? <p className={styles.MSG}>{msg.current}</p> : msgVisibile ? <p className={`${styles.MSG} ${styles.SuccessMsg}`}>{msg.current}</p> : null}
            </div>
        </Fragment>
    );
}
